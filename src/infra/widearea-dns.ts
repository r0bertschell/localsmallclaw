...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*localsmallclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  ...
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "lsmallclaw-gw");
  ...
  records.push(`_lsmallclaw-gw._tcp IN PTR ${instanceLabel}._lsmallclaw-gw._tcp`);
  records.push(`${instanceLabel}._lsmallclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._lsmallclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; localsmallclaw-content-hash: ${contentHash}\n${contentBody}`;
}
