...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `${STATE_DIRNAME}${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  const stateDir =
    env.LOCALSMALLCLAW_HOME?.trim() ||
    env.OPENCLAW_STATE_DIR?.trim() ||
    resolveProfileStateDir(profile, env, homedir);
  if (!env.LOCALSMALLCLAW_HOME?.trim() && !env.OPENCLAW_STATE_DIR?.trim()) {
    env.OPENCLAW_STATE_DIR = stateDir;
  }

  if (!env.LOCALSMALLCLAW_CONFIG?.trim() && !env.OPENCLAW_CONFIG_PATH?.trim()) {
    env.OPENCLAW_CONFIG_PATH = path.join(stateDir, CONFIG_FILENAME);
  }
